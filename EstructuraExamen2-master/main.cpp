...
int RecorrerArbol(NodoTrinario *raiz) {
    int numero=0;
   while(raiz != NULL){
   numero = raiz->num;
   return numero;
...
{
    int mayor1,mayor2,mayor3=0;
    int mayor=0;
    if(raiz == NULL)
        return mayor=raiz->num;

// mayor=RecorrerArbol(raiz->num);
 mayor1=RecorrerArbol(raiz->hijo_der);
 mayor2=RecorrerArbol(raiz->hijo_medio);
 mayor3=RecorrerArbol(raiz->hijo_izq);
    ...
    if(mayor3 > mayor1 && mayor3 > mayor2){
        return mayor3;
    }
}
